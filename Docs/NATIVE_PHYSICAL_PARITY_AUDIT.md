...
5. El único ABI v5 transporta `PanelLightSpreadProfile`, geometría relativa, cover/environment y camera/lens/sensor; no debe restaurarse una frontera insuficiente.
...
| Entorno HDR sintético, reflejos y rotación | Reflejos estructurados, color y contraste aparente. | `ProceduralEnvironment`, `EnvironmentPattern`, `ENVIRONMENT_PRESETS`, shaders spatial y physical. | Implementado/conectado CPU+Metal/Slint con biblioteca analítica neutral, softboxes, grid, office ceiling, daylight window, warm practicals y mixed production. | Todos los patterns + rotación 37°, roughness y parity. | ACEScg radiance → reflected image-plane illuminance. | ambient/key RGB radiance 0–100000, direction, angular radius degrees, rotation ±180°, strength 0–4; siete patrones publicados. | Solo entra por cover Fresnel; se suma después de transmisión. | Conectado por el snapshot físico actual. | Reusar `screen-cover`; una futura HDRI reemplaza el sampler bajo el mismo owner, no crea otra ruta. | Analítico rápido; roughness redistribuye hacia la media aproximada propia del patrón. | Biblioteca de presets disponible; no hay editor de mapa HDRI. | Cero exacto, rotación, cada pattern, no temporal modulation, CPU/Metal. |
...
1. Mantener la materialización completa de `PanelLightSpreadProfile` del ABI v5 sin lookup de preset, adaptador o default implícito.
