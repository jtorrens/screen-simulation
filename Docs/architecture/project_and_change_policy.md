...
Persistence validates document structure, stable opaque identifiers, portable paths, exact-time representation, ordering and cross-document references. Camera intrinsics documents carry every authored lens coefficient, RGB transmission and vignetting parameter explicitly. Persistence does not evaluate media, color, panel response, lens invertibility, camera projection or simulation. The composition root translates validated documents into the owning domain types; those domains remain authoritative for their semantics.
