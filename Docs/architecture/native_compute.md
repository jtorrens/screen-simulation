...
colorimetry, cover, its physical-radius core/tail glow approximation and exactly one rotated analytic or exact single-level equirectangular HDR environment. Image-backed rough reflection uses deterministic view-dependent GGX integration with exact dielectric Fresnel and Smith masking. The Metal estimator balances visible-normal samples with solid-angle-correct source-luminance samples. Platform prepares a private RGBA32F weight hierarchy once per accepted environment texture; its level-zero RGB remains the exact input radiance and higher levels contain only summed selection weights. Quality selects 256, 512, 1024 or 4096 samples of that same evaluator. The plan also carries either the
