...
Color owns application OCIO processing, but Fusion Scene Package exports no OCIO configuration. The writer delivers Device and Spill in the same selected implemented encoding and creates an explicit native Fusion 21 `AcesTransform` on each Loader to ACEScg. The saved ACES 2.0 Rec.709 D65 100 nit Reference uses `ACES_VERSION_2_0_0`, `IDT_REC709_100_INV_ODT` to `ODT_ACESCG`; another authored Reference transform produces that named node disabled with `PassThrough = true` and no substitute. There is no `ColorSpaceTransform`, OCIO, LUT or inferred route. The expanded job-specific Sticky Note records both media, Reference, metadata, exact alpha and transform contracts, ACEScg working space, composition equation and manual Viewer transform; it never changes values or claims to persist Viewer state.
