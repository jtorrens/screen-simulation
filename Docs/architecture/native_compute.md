...
colorimetry, cover, its physical-radius core/tail glow approximation and exactly one rotated analytic or exact single-level equirectangular HDR environment. Image-backed rough reflection uses deterministic view-dependent GGX visible-normal integration with exact dielectric Fresnel and Smith masking; quality selects 16, 32, 64 or 256 samples of that same evaluator. The plan also carries either the
