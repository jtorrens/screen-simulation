...
The product evaluator separately exposes the typed headless-only `device-vfx-transparency-v1` publication used by Fusion Scene Package. Its request carries one centred active frontal raster and one explicit DOF mode: disabled, baked, or Fusion. It is the normal Camera Rendered ACEScg route with only explicit export overrides: Metal maps the raster into Device-local UV, bypasses perspective and Brown distortion, uses one shutter sample rather than motion accumulation, and evaluates depth of field only in baked mode. Panel, cover, glow, lens transmission/vignetting/chromatic character/veiling glare, shutter exposure, sensor development and Camera Rendering Intent remain authored normal-render operations. The virtual sensor raster is exactly the requested padded export raster, so the route never applies an inferred rescale. Application then publishes typed Device and Spill rasters from the resolved rounded carrier geometry, never from `A == 0` or `A != 0`: Device keeps complete fractional matte and carrier RGB, Spill keeps exterior additive RGB over opaque black, and their RGB sum exactly reconstructs the returned physical result. Source overscan is resolved once before the first frame from maximum Device sampling density plus panel tail, full positive glow, optional baked DOF and the final radiometric fade. Every frame is evaluated once; the scene-linear threshold is metadata only and the fade attenuates only exterior Spill RGB. Camera motion blur is reconstructed from exported curves and shutter angle/phase. Partial Screen or Panel Emission requests fail because this route cannot consult Source or rebuild omitted radiometry.

Color owns application OCIO processing, but Fusion Scene Package exports no OCIO configuration. The writer delivers Device and Spill in the same selected encoding while format remains an independent alpha-capable OpenEXR, TIFF or ProRes choice. Device embeds its physical occlusion matte in alpha for every selected format that supports alpha. Physical-linear Spill is an opaque RGB medium already composed over black. Editorial Add Spill retains its declared straight alpha 0.125 in the delivered medium; the generated Fusion composition adds that RGB over an opaque black Background before ImagePlane3D, changing only the plane alpha to one and never scaling its additive RGB. Fusion carries Device as one RGBA 3D branch and Spill as one independent additive RGB 3D branch, preserves Device A when adding the projected Spill, and has no opaque Device-RGB conversion, separate matte carrier, matte renderer or recombination route. Each Loader receives an explicit native Fusion 21 conversion to ACEScg: `AcesTransform` for ACES and its exact supported camera Log/Gamut ids, or `ColorSpaceTransform` for DCM colorimetric and DaVinci Intermediate encodings with every gamut, gamma and disabled mapping field explicit. The saved ACES 2.0 Rec.709 D65 100 nit Reference uses `ACES_VERSION_2_0_0`, `IDT_REC709_100_INV_ODT` to `ODT_ACESCG`; another authored Reference transform produces that named node disabled with `PassThrough = true` and no substitute. There is no OCIO node, LUT or inferred route. The expanded job-specific Sticky Note records both media, Reference, metadata, exact alpha and transform contracts, ACEScg working space, composition equation and manual Viewer transform; it never changes values or claims to persist Viewer state.
