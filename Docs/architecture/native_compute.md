...
Color owns application OCIO processing, but Fusion Scene Package exports no OCIO configuration. The writer delivers Device and Spill in the same selected encoding while format remains an independent alpha-capable OpenEXR, TIFF or ProRes choice. Both Loaders preserve straight authored RGBA. Before native Fusion 21 color conversion, a `Custom` node supplies the media RGB with alpha one; after `AcesTransform` or `ColorSpaceTransform`, a second `Custom` node multiplies transformed RGB by the untouched Loader alpha and restores that alpha. Device thus publishes one premultiplied ACEScg contribution with its physical matte. Physical-linear Spill alpha one remains identity; Editorial Add Spill alpha 0.125 attenuates its ACEScg contribution after, never during, nonlinear color conversion. Fusion carries Device as one RGBA 3D branch and Spill as one independent additive RGB 3D branch, preserves Device A when adding projected Spill, and has no separate matte carrier, matte renderer or recombination route. External Reference first crosses native `ChangeDepth` with `Depth = 3`, guaranteeing at least float16 before its color transform. The saved ACES 2.0 Rec.709 D65 100 nit Reference then uses `ACES_VERSION_2_0_0`, `IDT_REC709_100_INV_ODT` to `ODT_ACESCG`; another authored Reference transform produces that named node disabled with `PassThrough = true` and no substitute. There is no OCIO node, LUT or inferred route. The expanded job-specific Sticky Note records both media, Reference, metadata, exact alpha and transform contracts, ACEScg working space, composition equation and manual Viewer transform; it never changes values or claims to persist Viewer state.
