...
The incident-environment port accepts exactly one resolved source: a complete synthetic linear-HDR profile or a decoded equirectangular radiance map already transformed explicitly to ACEScg. Synthetic distributions include the current production and diagnostic patterns. An image source additionally owns its cd/m²-per-linear-unit calibration, exposure in stops and horizontal rotation; its raster must be finite, 2:1 and present exactly when that source is selected. Environment strength zero is exact absence. Nonzero radiance is added only through the cover interface reflection and never modulates or replaces panel emission. Roughness and haze broaden synthetic structure toward its bounded mean. For image-backed radiance, the platform prepares one deterministic energy-normalized angular-diffusion pyramid from the accepted level-zero sphere. Level zero is an exact copy; every coarser level accumulates a normalized Gaussian diffusion with equirectangular latitude correction, followed at that represented scale by a small positive normalized separable scatter that removes discrete reconstruction contours without touching panel emission. A source therefore remains one continuous reflected source rather than fragmenting into sparse sampling replicas. A normalized two-lobe lookup represents the defined core and broad scatter tail: the core width is perceptually spaced in angle, while the tail combines the rough-surface angular radius with the independent haze radius. The tail probability is the union of rough-surface and haze scattering, so core and tail weights sum exactly to one. At normal incidence each lobe is radial; oblique incidence stretches it continuously in the local incidence-plane direction by the bounded reciprocal view cosine, with a smooth onset once the additional support becomes representable at the selected pyramid level. Both angular footprints are independent of source resolution, authored zero remains exactly sharp, and constant incident radiance is preserved at every material value. Both sources enter the same Fresnel/transmission evaluator.
