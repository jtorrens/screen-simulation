...
The incident-environment port accepts exactly one resolved source: a complete synthetic linear-HDR profile or a decoded equirectangular radiance map already transformed explicitly to ACEScg. Synthetic distributions include the current production and diagnostic patterns. An image source additionally owns its cd/m²-per-linear-unit calibration, exposure in stops and horizontal rotation; its raster must be finite, 2:1 and present exactly when that source is selected. Environment strength zero is exact absence. Nonzero radiance is added only through the cover interface reflection and never modulates or replaces panel emission. Roughness and haze broaden synthetic structure toward its bounded mean. Image-backed reflection is evaluated directly from the exact single-level radiance map with the resolved view direction. The reference evaluator importance-samples visible Trowbridge-Reitz/GGX microfacet normals, applies exact unpolarized dielectric Fresnel and Smith masking, and integrates the reflected incident radiance without a screen-space blur, angular mip pyramid or manually stretched halo. Oblique highlight shape therefore follows the view-dependent microfacet geometry rather than a fitted circle or ellipse. A deterministic per-output-pixel Cranley shift decorrelates the shared Hammersley sequence without frame noise; Draft, Medium, High and Native use respectively 16, 32, 64 and 256 samples of the same model. Authored zero roughness remains the exact mirror lookup. Haze remains the separately owned bounded transmission/scatter approximation and is not presented as a second GGX roughness. Both environment source kinds enter the same cover-interface response.
