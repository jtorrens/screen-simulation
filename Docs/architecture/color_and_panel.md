...
The current environment is an explicit ACEScg linear-radiance profile with ambient illumination and one authored spherical analytic pattern: Uniform Neutral, Studio Softboxes, Calibration Grid, Office Ceiling, Daylight Window, Warm Practicals or Mixed Production. Mixed Production has complete presets for a generic production set and for a warm practical-lit interior with a cooler doorway contribution. These are incident-radiance distributions, not alternative reflection algorithms, and make cover response, focus, roughness and common production feels directly testable. A future HDR adapter replaces only that authored incident-radiance source and must enter the same `screen-cover` evaluator. It cannot introduce another reflection operator. Cover and environment character amounts are multiplicative authoring controls: zero cover is an exact identity, zero environment contributes no reflection and values above one deliberately exaggerate the current material response.
