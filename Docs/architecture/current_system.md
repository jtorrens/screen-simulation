...
The technical desktop camera-result view authors one complete capture profile: native photosite raster, physical gate, Bayer pattern, sensor response/noise/ADC data, one lens-association policy, one explicit default lens id and a nonempty compatible-lens set. The bundled templates are ARRI ALEXA 35 Open Gate and an explicitly labelled calibrated approximation of the iPhone 16e main camera. Every optic, including an integrated handset module, lives in the single Geometry-owned lens catalog. The ARRI declares the compatible generic 18, 25, 35, 50, 85 and 135 mm approximations and an interchangeable policy. The iPhone declares only its calibrated integrated 4.2 mm main lens and a fixed policy; it does not emulate computational photography. Selecting a camera and compatible objective copies their separate complete values into current state, and evaluation never resolves a preset id. Focal length, distortion, chromatic behavior, vignetting, transmission and softness come only from the selected lens, never from a duplicate camera field.
