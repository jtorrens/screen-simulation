...
→ energy-conserving physical panel light spread
...
The project-owned physical state includes complete LCD, panel-light-spread, optical-cover, environment and sensor profiles. Panel light spread, cover and environment cross the same immutable Application request as panel and sensor; they are not workstation-only effects.
...
panel light spread, cover plus incident environment, lens, then the discrete CFA and demosaic boundary. Panel and lens
...
The bundled LCD device catalog is a set of current authoring templates with stable ids, native raster, active physical dimensions, one reference operating white with an explicit basis label, one complete panel-light-spread profile and one default optical-cover authoring template. Selecting a device copies all complete profiles into current state; it never leaves a runtime dependency on any catalog id. The white, light-spread amount and cover selection remain editable. Evaluation never looks up a preset id, and an existing project result therefore cannot change when the bundled catalog changes. Generic templates state that their white and panel transport are authored approximations; the ASUS ProArt PA329CV uses its published 350-nit typical SDR value and the 14.2-inch MacBook Pro template uses its published 500-nit SDR value. `Custom` edits the same complete values and does not select another evaluator. PPI, diagonal and pixel pitch are derived presentation values, never independent authorities.
