...
The optical-cover catalog provides glossy, semi-gloss, matte and thick-glass approximations. Its complete material profile authors thickness, refractive index, anti-reflective efficiency, RGB absorption, roughness and haze. A separate environment profile supplies linear ambient radiance plus one finite angular key source. The desktop exposes independent non-negative character amounts: cover amount zero is an exact identity and environment amount zero contributes no reflection, while values above one explicitly exaggerate the same operators. The procedural environment is the current evaluable source; a later HDR image adapter must populate this same environment boundary rather than create another reflection path. Cover evaluation is shared by Draft, Medium, High and Native capture.
