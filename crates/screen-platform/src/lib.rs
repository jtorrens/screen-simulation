...
mod ffmpeg_media {
    ...
    use screen_media::{
        AlphaPresence, DecodedFrame, DecodedRgba, FrameCadence, FrameSelectionPolicy,
        MediaDescriptor, PixelEncoding, RasterSize, ResolvedSignalRange, ResolvedSourceDecode,
        ResolvedYuvMatrix,
    };
    ...
    pub fn decode_frame_at_time(
        ...
        match descriptor.duration {
            Some(duration)
                if requested_time >= duration && policy == FrameSelectionPolicy::Exact =>
            {
    ...
    fn earlier_is_nearer_or_tied(
        ...
        let earlier_numerator = (i128::from(requested.numerator())
            * i128::from(earlier.denominator())
        ...
        .abs();
        let later_denominator =
            i128::from(later.denominator()) * i128::from(requested.denominator());
    ...
    mod ffmpeg_bridge {
        ...
        pub(super) fn pixel_encoding(
            descriptor: ffmpeg::format::pixel::Descriptor,
        ) -> PixelEncoding {
            ...
            if flags
                & u64::try_from(ffmpeg::ffi::AV_PIX_FMT_FLAG_RGB).expect("RGB flag is positive")
                != 0
            {
                PixelEncoding::Rgb
            } else {
    ...
    mod tests {
        ...
        fn ffmpeg_color_declarations_map_to_typed_metadata_without_selecting_an_idt() {
            use ffmpeg::util::color::{
                Primaries, Range, Space, TransferCharacteristic as Transfer,
            };
