...
enum FusionSpillSupport {
    ...
    static func prepare(
        ...
        let active = source.activeRect
        let measuredSupport = try measureThresholdSupport(
            source, thresholdSceneLinear: thresholdSceneLinear
        )
        let thresholdSupport = fixedThresholdSupportPixels
        guard thresholdSupport >= measuredSupport else {
...
struct FusionScenePackageRequest: Equatable, Sendable {
    ...
    func validate() throws {
        ...
        guard configuration.outputType == .fusionScenePackage,
              ...
              configuration.frameRate.isFinite, configuration.frameRate > 0,
              deviceWidthMeters.isFinite, deviceWidthMeters > 0,
              deviceHeightMeters.isFinite, deviceHeightMeters > 0,
              ...
              motionBlur.shutterPhaseDegrees.isFinite else {
            throw FusionScenePackageError.invalidCamera
        }
...
enum FusionScenePackageWriter {
    ...
    static func render(
        ...
        var fixedThresholdSupport = 0
        var prepassRaster: (width: Int, height: Int, active: FusionRasterRect)?
        for frame in frames {
            try Task.checkCancellation()
            let raw = try await frameProvider(frame)
            try raw.validate()
            guard raw.activeRect.width == request.activeRaster.activeWidth,
                  raw.activeRect.height == request.activeRaster.activeHeight else {
                throw FusionScenePackageError.invalidRaster
            }
            if let prepassRaster {
                guard prepassRaster.width == raw.width,
                      prepassRaster.height == raw.height,
                      prepassRaster.active == raw.activeRect else {
                    throw FusionScenePackageError.invalidRaster
                }
            } else {
                prepassRaster = (raw.width, raw.height, raw.activeRect)
            }
            fixedThresholdSupport = max(
                fixedThresholdSupport,
                try FusionSpillSupport.measureThresholdSupport(
                    raw, thresholdSceneLinear: options.spillThresholdSceneLinear
                )
            )
        }
    ...
    static func metadata(
        ...
        return FusionSceneMetadata(
            ...
            spill: .init(
                ...
                sourceOverscanPixels: prepared.activeRect.x,
            ...
            lensReconstruction: .init(
                bakedInEXR: false,
                contract: "fusion-native-lens-distort-brown-conrady-v1",
                fusionTool: "LensDistort",
                fusionModel: "FusionRadial",
                parameterMapping: "k1/k2/k3 -> LowOrder/HighOrder/FishEye; p2/p1 -> Tangential X/Y"
            ),
    ...
    static func fusionComp(
        ...
        let media = "Comp:/../media/\(configuration.jobName).%08d.exr"
        ...
        let lensK1 = fusionSpline(request.lens.map { ($0.frame, $0.radialK1K2K3[0]) })
        let lensK2 = fusionSpline(request.lens.map { ($0.frame, $0.radialK1K2K3[1]) })
        let lensK3 = fusionSpline(request.lens.map { ($0.frame, $0.radialK1K2K3[2]) })
        let lensP1 = fusionSpline(request.lens.map { ($0.frame, $0.tangentialP1P2[0]) })
        let lensP2 = fusionSpline(request.lens.map { ($0.frame, $0.tangentialP1P2[1]) })
        let firstLens = request.lens[0]
        let lensCenterX = 0.5 + firstLens.opticalCenterXY[0]
        let lensCenterY = 0.5 + firstLens.opticalCenterXY[1]
        ...
        // The two CustomTools implement the physical equation channel by channel. A Merge/Over
        // node is intentionally absent, so RGB outside the matte cannot be suppressed.
        return """
        {
          Attrs = { COMPN_GlobalStart = \(configuration.firstFrame), COMPN_GlobalEnd = \(configuration.lastFrame) },
          Tools = ordered() {
            DeviceRGBA = Loader {
              Clips = { Clip { Filename = "\(media)", StartFrame = \(configuration.firstFrame) } },
            ...
            DevicePlane = ImagePlane3D {
              Inputs = {
                MaterialInput = Input { SourceOp = "DeviceRGBA", Source = "Output" },
                ["Transform3DOp.Scale.X"] = Input { Value = \(planeWidth * 0.5) },
                ["Transform3DOp.Scale.Y"] = Input { Value = \(planeHeight * 0.5) },
                ["Transform3DOp.Scale.Z"] = Input { Value = 1 }
              },
              CustomData = { TransformContract = "identity-at-origin", WidthMeters = \(planeWidth), HeightMeters = \(planeHeight), ActiveRect = "\(prepared.activeRect.x),\(prepared.activeRect.y),\(prepared.activeRect.width),\(prepared.activeRect.height)" }
            },
            Camera3D_Device = Camera3D {
              Inputs = {
                ...
                FStop = Input { SourceOp = "CameraFStop", Source = "Value" },
                ...
                Near = Input { Value = \(firstCamera.nearClipMeters) },
                Far = Input { Value = \(firstCamera.farClipMeters) },
                EnableDepthOfField = Input { Value = \(dofEnabled) }
              },
              CustomData = { FStopCurve = "CameraFStop", ExactQuaternionCurves = "Comp:/../metadata/\(configuration.jobName)_FusionScene.json" }
            },
            ...
            LensK1 = BezierSpline { KeyFrames = { \(lensK1) } },
            LensK2 = BezierSpline { KeyFrames = { \(lensK2) } },
            LensK3 = BezierSpline { KeyFrames = { \(lensK3) } },
            LensP1 = BezierSpline { KeyFrames = { \(lensP1) } },
            LensP2 = BezierSpline { KeyFrames = { \(lensP2) } },
            Scene3D = Merge3D {
              Inputs = {
                SceneInput1 = Input { SourceOp = "DevicePlane", Source = "Output" },
                SceneInput2 = Input { SourceOp = "Camera3D_Device", Source = "Output" }
              }
            },
            RenderDevice = Renderer3D {
              Inputs = {
                SceneInput = Input { SourceOp = "Scene3D", Source = "Output" },
                CameraSelector = Input { Value = "Camera3D_Device" },
                EnableDepthOfField = Input { Value = \(dofEnabled) },
                MotionBlur = Input { Value = 1 },
                ShutterAngle = Input { Value = \(request.motionBlur.shutterAngleDegrees) },
                ShutterPhase = Input { Value = \(request.motionBlur.shutterPhaseDegrees) }
              }
            },
            DeviceLensDistortion = LensDistort {
              Inputs = {
                ...
                ["FusionRadial.LowOrderDistortion"] = Input { SourceOp = "LensK1", Source = "Value" },
                ["FusionRadial.HighOrderDistortion"] = Input { SourceOp = "LensK2", Source = "Value" },
                ["FusionRadial.FishEyeDistortion"] = Input { SourceOp = "LensK3", Source = "Value" },
                ["FusionRadial.TangentialDistortion.X"] = Input { SourceOp = "LensP2", Source = "Value" },
                ["FusionRadial.TangentialDistortion.Y"] = Input { SourceOp = "LensP1", Source = "Value" },
                ["FusionRadial.PixelAspect"] = Input { Value = { 1, 1 } },
                ["FusionRadial.LensCenter"] = Input { Value = { \(lensCenterX), \(lensCenterY) } },
                ...
                LensShiftX = Input { SourceOp = "CameraLensShiftX", Source = "Value" },
                LensShiftY = Input { SourceOp = "CameraLensShiftY", Source = "Value" },
                Input = Input { SourceOp = "RenderDevice", Source = "Output" }
              },
              CustomData = { Contract = "fusion-native-lens-distort-brown-conrady-v1", LensBakedInEXR = false }
            },
            PlateInput = Background {
              CustomData = { Role = "CONNECT_PLATE_HERE", ColorSpace = "ACEScg scene-linear" }
            },
            PlateOccluded = CustomTool {
              Inputs = {
                Image1 = Input { SourceOp = "PlateInput", Source = "Output" },
                Image2 = Input { SourceOp = "DeviceLensDistortion", Source = "Output" },
                RedExpression = Input { Value = "r1*(1-a2)" },
                GreenExpression = Input { Value = "g1*(1-a2)" },
                BlueExpression = Input { Value = "b1*(1-a2)" },
                AlphaExpression = Input { Value = "a1" }
              }
            },
            PhysicalComposite = CustomTool {
              Inputs = {
                Image1 = Input { SourceOp = "DeviceLensDistortion", Source = "Output" },
                Image2 = Input { SourceOp = "PlateOccluded", Source = "Output" },
    ...
    private static func fusionEulerDegrees(_ quaternion: [Double]) -> SIMD3<Double> {
        let x = quaternion[0]
        let y = quaternion[1]
        let z = quaternion[2]
        let w = quaternion[3]
